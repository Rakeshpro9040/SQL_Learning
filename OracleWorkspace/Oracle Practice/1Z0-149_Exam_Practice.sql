...
---------------------- Trigger ----------------------
drop table t; -- This will drop the trigger
drop table t_temp;

create table t (n number);
create table t_temp (ts timestamp);
...
for each row
begin
insert into t_temp values (systimestamp);
end t_trig;
/
-- Trigger T_TRIG compiled
select * from user_objects where object_name = 'T_TRIG'; -- VALID

insert into t values (1);

-- Current Session
select * from t; -- 1
select * from t_temp; -- 18-OCT-22 04.20.25.815612000 AM

-- New Session
-- no Data
COMMIT;
-- now data is avilable
...
-- Current Session
select * from t; -- 3
select * from t_temp; -- 18-OCT-22 05.32.28.195452000 AM

-- New Session
select * from t; -- no new data
select * from t_temp; -- 18-OCT-22 05.32.28.195452000 AM
COMMIT;
select * from t; -- 3
select * from t_temp; -- 18-OCT-22 05.32.28.195452000 AM
...
for each row
begin
execute immediate 'create table t_bkp (n number)';
end t_trig;
/
-- Trigger T_TRIG compiled
insert into t values (1);
...
select * from user_objects where object_name = 'T_BKP'; -- no object
select * from t; -- no new data
